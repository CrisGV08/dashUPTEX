...
def titulados_historico_actual_view(request):
    ...
    generaciones_qs = GeneracionCarrera.objects.select_related(
        'programa_antiguo', 'programa_nuevo'
    )

    if anio:
        generaciones_qs = generaciones_qs.filter(fecha_ingreso__year=anio)

    if carrera:
        generaciones_qs = generaciones_qs.filter(
            programa_antiguo_id__in=carrera
        ) | generaciones_qs.filter(
            programa_nuevo_id__in=carrera
        )

    generaciones = generaciones_qs.order_by('fecha_ingreso')

    # ========================
    # FILTROS DISPONIBLES
    # ========================
    todos = GeneracionCarrera.objects.select_related('programa_antiguo', 'programa_nuevo').all()
    anios_disponibles = sorted(set(g.fecha_ingreso.year for g in todos))

    claves_antiguos = ProgramaEducativoAntiguo.objects.values_list('id', 'nombre')
    claves_nuevos = ProgramaEducativoNuevo.objects.values_list('id', 'nombre')
    carreras_disponibles = list(claves_antiguos) + list(claves_nuevos)
    ...
    return render(request, 'titulados_historico_actual.html', {
        ...
        'carreras_seleccionadas': carrera,
    })
